...
if __name__ == "__main__":
    ...
    logger.info(f"  - Обработано групп Y360: {members_stats.get('y360_groups_processed', 0)}")
    logger.info(f"  - Найдено групп в AD: {members_stats.get('ad_groups_found', 0)}")
    logger.info(f"  - Пропущено групп: {members_stats.get('skipped_groups_count', 0)}")
    logger.info(f"  - Добавлено участников: {members_stats.get('users_added_count', 0)}")
    logger.info(f"  - Удалено участников: {members_stats.get('users_removed_count', 0)}")
    logger.info(f"  - Пользователей не найдено в Y360: {members_stats.get('users_not_found_count', 0)}")
    logger.info(f"  - Ошибок: {members_stats.get('errors_count', 0)}")
    logger.info("=" * 80)
